...
nnet_fit <- 
  mlp(hidden_units = 5, penalty = 0.1) %>% 
  set_mode("classification") %>% 
  fit(disease ~ ., data = lab_data)
```
:::
...
## Or another model that you've never used
...
mars_grid <- augment(mars_fit, grid)
p + 
  geom_contour(data = mars_grid, aes(z = .pred_yes), 
               breaks =  1 / 2, col = "black", linewidth = 1.2)
```
...
nnet_tune <- 
  ...
  set_mode("classification") %>% 
  tune_grid(disease ~ ., resamples= vfold_cv(lab_data), control = ctrl, grid = 10)
...
animate(nnet_anime, width = 400, height = 400, res = 72*1.2, dev = "png") 
...
augment(rf_fit, churn_te %>% slice(1:5), eval_time = 1:200) %>% 
  ...
  unnest(cols = .pred) %>%
  mutate(
    employee = LETTERS[.row],
    .prob_churn = 1 - .pred_survival
  ) %>% 
  ggplot(aes(.eval_time, .prob_churn, col = employee, group = employee)) + 
